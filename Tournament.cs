...
namespace RpgCharacters
{
    public class Tournament
    {
        public Tournament()
        {
            ...
            Console.WriteLine("Choose your characters and let the battle begin!\nHow many characters do you want to create? (Maximum 10)");
            ...
            {
                Console.WriteLine($"Enter name for Character {i + 1}:");
                string characterName = Console.ReadLine() ?? $"Character{i + 1}";
                Console.WriteLine($"Choose races for Character {i + 1}:\n1. Fairy\n2. Orc\n3. Elf\n4. Human");
        ...
        static void Attack(RpgCharacters.Character attacker, RpgCharacters.Character defender, Random rng)
        {
            ...
            int damageDealt = (int)Math.Max(damageTo2 - defense2, 0);
            defender.Hp -= damageDealt;

            Console.WriteLine($"{attacker.Name} attacks {defender.Name} attack: {damageTo2:0.00}. {defender.Name} defense: {defense2:0.00}.");
            Console.WriteLine($"{attacker.Name} deals {damageDealt} damage to {defender.Name}. {defender.Name} HP: {defender.Hp}");
        }  
    }
}